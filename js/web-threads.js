/**
 * WebThreads Component (React Bits Native OGL Implementation)
 * Renders glowing interactive woven threads with WebGL2 via OGL
 */
...
export function initWebThreads(containerId, options = {}) {
  const container = document.getElementById(containerId);
  if (!container) return;

  const defaults = {
    color1: '#00A09A',       // Luxury Jade / Teal
    color2: '#34D399',       // Bright Mint
    color3: '#FFFFFF',       // Hot Core Light
    speed: 0.25,
    threadCount: 7,
    frequency: 4.5,
    spread: 0.22,
  ...
  try {
    ...
    console.warn('WebGL2 not supported or OGL initialization failed:', err);
    return;
  }
  ...
  const rgb3 = hexToRgb(config.color3);
  ...
  const program = new Program(gl, {
    ...
    uniforms: {
      ...
      uFanMode: { value: FAN_MODE[config.fanMode] ?? 0 },
  ...
  // Also support window-level mouse movement over section
...
document.addEventListener('DOMContentLoaded', () => {
  if (document.getElementById('hero-web-threads-container')) {
    initWebThreads('hero-web-threads-container');
  } else if (document.getElementById('web-threads-container')) {
    initWebThreads('web-threads-container');
  }
});
