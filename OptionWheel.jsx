...
const OptionWheel = ({
  ...
  const handleItemClick = useCallback(
    index => {
      ...
      if (isAlreadyCentered && onSelectRef.current) {
  ...
  const handleKeyDown = useCallback(
    e => {
      ...
      if (e.key === 'ArrowUp' || e.key === 'ArrowLeft') delta = -1;
        ...
        e.preventDefault();
        onSelectRef.current?.(selectedIndex, items[selectedIndex]);
        return;
      }
    ...
    [applyTarget, selectedIndex, items]
  );
  ...
  return (
    <div
      ...
      role="listbox"
      tabIndex={0}
      aria-label="Option wheel navigation"
